...
def compute_grid_fitness(candidate, target, clip_model, clip_processor, lpips_fn, device, grid=2):
    w, h = target.size
    rw, rh = w // grid, h // grid
    candidate_rs = candidate.convert("RGB").resize((w, h), Image.BILINEAR)
    ...
    for r in range(grid):
        ...
        for c in range(grid):
            ...
            m = compute_fitness(
                candidate_rs.crop(box), target_rgb.crop(box),
                clip_model, clip_processor, lpips_fn, device,
            )
